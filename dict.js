...
export const dictionaries = {
    'en': {
        ...
        'qra_opt_enable': 'Enable Plugin',
        'qra_opt_disable': 'Disable Plugin',
        ...
        'qra_card_log_desc': 'View plugin status, troubleshoot issues, or export logs',
        ...
        'qra_whitelist_help_p4': 'Icons: <i class="fa-solid fa-sun"></i> = XiaoBaiX script, <i class="fa-solid fa-star"></i> = Quick Reply, <i class="fa-solid fa-moon"></i> = JSR script.',
        ...
        // 更新弹窗
        'qra_modal_update_title': 'Plugin Update Log',
        'qra_update_tip': 'If the update fails, you can manually update QR Assistant from the Extensions management page. If it still fails, try deleting and reinstalling the plugin.',
        ...
        'qra_help_toc_compat': 'Plugin Compatibility',
    ...
    'zh-tw': {
        ...
        'qra_card_help_title': '外掛使用說明',
        'qra_card_help_desc': '檢視關於本外掛的詳細功能介紹與幫助',
        'qra_card_log_title': '系統日誌',
        'qra_card_log_desc': '檢視外掛執行狀態、排查問題或匯出日誌報告',
        ...
        'qra_whitelist_help_p4': '此外，<i class="fa-solid fa-sun"></i> 表示為小白X指令碼，<i class="fa-solid fa-star"></i> 表示為QR，而 <i class="fa-solid fa-moon"></i> 表示為酒館助手指令碼。',
        ...
        // 更新彈窗
        'qra_modal_update_title': '外掛更新日誌',
        'qra_update_tip': '如果更新失敗，可以在酒館擴充頁面的【管理擴充程式】列表中手動更新QR助手。如果依然更新失敗，則可以嘗試刪除重裝來使用最新版本外掛。',
...
export const helpDocs = {
    ...
    <blockquote>
        The plugin provides a <code>Whitelist</code> feature. Buttons in the whitelist will remain above the input bar instead of being moved to the menu, so you can selectively keep some buttons visible (e.g., Input Helper).
    </blockquote>
    ...
    <ul>
        <li>Click any QR, JSR script, or XiaoBaiX button entry to move it to the whitelist. Whitelisted buttons will remain above the chat input bar.</li>
    </ul>

    <h3 id="help-icons">Button Icon</h3>
    <h4>Built-in Icons</h4>
    <ul>
        <li>The default button icon is Star & Crescent. The plugin also offers Rocket, Hexagram, Palette, and many other built-in icons.</li>
    </ul>
    <h4>Custom Icons</h4>
    <ul>
        <li>In addition to built-in icons, the plugin supports <code>Custom Icons</code>.</li>
    ...
    <ul>
        <li>The plugin also supports using the <code>Current Character Card</code> avatar as the button icon. It updates automatically when switching chats.</li>
    </ul>
    <h4>Other Notes</h4>
    ...
    <ul>
        <li>The plugin includes multiple built-in UI themes for you to choose from.</li>
    </ul>

    <h3 id="help-sort">Menu Button Custom Sorting</h3>
    <ul>
        <li>The plugin supports custom sorting of menu buttons. Click the <code>Settings</code> button in the <code>Button Sorting</code> card on the main settings page to open the sort management dialog.</li>
    ...
    <ul><li>Use the <code>Enable/Disable Plugin</code> dropdown.</li></ul>
    ...
    <h3 id="help-compat">Input Helper & Preset Transfer Plugin</h3>
    <ul>
        <li>Add Input Helper (Script Version) to the whitelist to keep its symbol buttons visible.</li>
        <li>Compatible with the <code>Preset Transfer Tool</code> plugin — its "World Book Common" button will appear on the left side of the list.</li>
    </ul>

    <h3 id="help-update">Update Check</h3>
    <ul>
        <li>In addition to SillyTavern's built-in update check on the Extensions management page, the plugin also checks for new versions automatically every 24 hours. When a new version is available, a <code>NEW</code> badge will appear, and the <code>Check for Updates</code> button will turn red with the text <code>New Version Found</code>. Click it to view the changelog and quickly update the plugin.</li>
        <li>If the update fails, you can manually update QR Assistant from the Extensions management page. If it still fails, try deleting and reinstalling the plugin to use the latest version.</li>
    </ul>
...
`,
    ...
    <blockquote>
        外掛提供<code>白名單</code>功能，在<code>白名單</code>中的按鈕將不會放到該選單中。因此，你可以選擇性地保留一些按鈕到輸入框上方（例如輸入助手）。
    </blockquote>
    ...
    <h4>內建圖示</h4>
    <ul>
        <li>預設按鈕圖示是星月，不過外掛還內建了小火箭、六芒星、調色盤等圖示可供選擇。</li>
    </ul>
    <h4>自訂圖示</h4>
    <ul>
        <li>除了以上內建的圖示外，外掛還支援<code>自訂圖示</code>。</li>
    ...
    <h4>目前角色卡</h4>
    <ul>
        <li>外掛還支援使用<code>目前角色卡</code>的頭像作為按鈕圖示，切換聊天時會自動更新為對應角色的頭像。</li>
    </ul>
    <h4>其他說明</h4>
    ...
    <ul>
        <li>外掛內建多種UI主題，可供選擇。</li>
    </ul>

    <h3 id="help-sort">選單按鈕自訂排序</h3>
    <ul>
        <li>外掛支援自訂選單按鈕的排序。在主設定頁面點選<code>選單按鈕排序</code>卡片中的<code>設定</code>按鈕，即可開啟排序管理彈窗。</li>
    ...
    <h4>一鍵停用/啟用QR助手</h4>
    <ul><li>選擇<code>啟用/停用外掛</code>即可</li></ul>
    <h4>內建自動折疊輸入框區域</h4>
    <ul><li>啟用<code>按鈕自動折疊</code>功能即可。</li></ul>
    <h4>提供按鈕選單背景模糊效果</h4>
    <ul><li>啟用<code>背景模糊效果</code>功能即可。</li></ul>

    <h3 id="help-compat">輸入助手與預設轉移外掛</h3>
    <ul>
        <li>將輸入助手（指令碼版）移至白名單即可正常顯示對應的符號按鈕。</li>
        <li>已經相容<code>預設轉移工具</code>外掛的「世界書常用」按鈕，會顯示在列表左側。</li>
    </ul>

    <h3 id="help-update">檢測更新</h3>
    <ul>
        <li>除了酒館內建的在<code>管理擴充程式</code>頁面提供的檢測更新外，外掛也提供自動檢測版本更新。每超過24小時會檢測一次版本。如果存在最新版本，則會顯示<code>NEW</code>符號，並且<code>檢測版本</code>會變為紅色的<code>發現新版本</code>標題。點選<code>發現新版本</code>可檢視更新日誌，並快速進行更新外掛。</li>
        <li>如果更新失敗，可以在酒館擴充頁面的<code>管理擴充程式</code>列表中手動更新QR助手。如果依然更新失敗，則可以嘗試刪除重裝來使用最新版本外掛。</li>
    </ul>
    <div style="height: 100px;"></div>
`,
...
export const announcementDocs = {
    ...
    <ul style="padding-left: 20px; margin-top: 0;">
        ...
        <li>Finally, detailed information and the manual of the plugin can be found in the "Usage Guide" on the settings page.</li>
    </ul>
    `,
    ...
    <ul style="padding-left: 20px; margin-top: 0;">
        <li>現在支援將選單面板中的按鈕進行自訂排序了，在QR助手的外掛設定頁面打開「選單按鈕排序」並開啟該功能，並進行自訂排序即可。</li>
    ...
    <ul style="padding-left: 20px; margin-top: 0;">
        ...
        <li>最後，關於外掛的詳細資訊和說明頁可以在外掛頁面的「使用說明」中查看。</li>
    </ul>
    `,
