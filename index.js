...
import { createSettingsHtml, createModalOverlayHtml, loadAndApplySettings, setupEventListeners, refreshUpdateUI, populateWhitelistManagementUI } from './settings.js';
...
function ensureSettingsInitialized() {
    ...
    if (!currentSettings.migrated_from_v1) {
        console.log(`[${Constants.EXTENSION_NAME}] 正在初始化新环境并检测旧版本数据...`);

        if (oldSettings) {
            console.log(`[${Constants.EXTENSION_NAME}] 检测到旧版数据，开始跨键名静默克隆...`);
            ...
            // 新增：如果是从旧版 V1 迁移过来的用户，视为老用户，默认跳过首次设置向导
            ...
            // 如果旧版是 fontawesome 类型，由于新版不支持，强制回退至默认小火箭
            if (oldSettings.iconType === 'fontawesome') {
                currentSettings.iconType = Constants.ICON_TYPES.ROCKET;
            } else {
                currentSettings.iconType = oldSettings.iconType || Constants.ICON_TYPES.ROCKET;
            }
        }
        ...
        if (typeof context.saveSettingsDebounced === 'function') {
            ...
            console.log(`[${Constants.EXTENSION_NAME}] 迁移数据已成功锁定并写入磁盘。`);
        }
    }
    ...
    try {
        ...
        setTimeout(() => {
            if (typeof refreshUpdateUI === 'function') {
                refreshUpdateUI(false).catch(e => console.warn("Auto-update check failed:", e));
            }
...
function performInitialization() {
    ...
    // 修复：添加设置抽屉的点击监听器，打开面板时自动刷新白名单列表
