...
function showCustomIconGuidance(menuContainer) {
    ...
    overlay.innerHTML = `
        <div class="qr-ob-guidance-box">
            <div class="qr-ob-guidance-title">新增自定义图标</div>
            <div class="qr-ob-guidance-text">
                请点击酒馆顶部菜单栏的 <i class="fa-solid fa-cubes"></i> 扩展管理图标，找到 <strong>【QR助手】</strong>，切换到 <strong>"图标按钮"</strong> 页面新增并启用自定义图标。
            </div>
            <button class="qr-ob-btn" id="qr-ob-guidance-close">知道了</button>
        </div>
    `;
...
function renderOnboardingHeader(menuContainer, settings) {
    ...
    header.innerHTML = `
        ...
        <div class="qr-ob-group qr-ob-icon-source">
            <button id="${Constants.ID_ONBOARDING_ICON_SOURCE}" class="qr-ob-icon-btn" title="切换图标来源">
