...
export function loadAndApplySettings() {
    ...
    // 初始化滑块位置
    setTimeout(() => {
        const activeTab = document.querySelector('.qr-settings-tab-item.active');
...
export function populateWhitelistManagementUI() {
    ...
    map.forEach((name, id) => {
        ...
        div.addEventListener('mouseenter', (e) => {
            ...
            showTooltip(e, mockReply, true); // true 表示只显示来源和范围
        });
...
export function setupEventListeners() {
    ...
    const updateGliderPosition = (targetTab) => {
        const tabBar = document.getElementById(Constants.ID_SETTINGS_TAB_BAR);
        const glider = tabBar?.querySelector('.qr-tab-glider');
        if (targetTab && glider) {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('click', (e) => {
            ...
            // 移动滑块
            updateGliderPosition(currentTab);
