            ...
            <div class="inline-drawer-content">
                ...
                <div id="${Constants.ID_PAGE_MAIN}" class="qr-settings-page active">
                    ...
                    <div class="flex-container flexGap5" style="margin-top: 10px; align-items: center;">
                        ...
                        <select id="${Constants.ID_UI_THEME_DROPDOWN}" class="text_pole" style="flex:1;">
                            ...
                            <option value="${Constants.UI_THEMES.CUTE}">樱花</option>
                            <option value="${Constants.UI_THEMES.CLASSICAL}">古典</option>
                            <option value="${Constants.UI_THEMES.DARK}">暗黑</option>
                        ...
                        <div class="qr-setting-card" style="align-items: flex-start;">
                            ...
                            <div style="display: flex; flex-direction: column; gap: 5px; min-width: 140px;">
                                <select id="qr-log-level-select" class="text_pole" style="width: 100%;">
                                    ...
                                    <option value="1">INFO (常规)</option>
                                    <option value="2">WARN (仅警告)</option>
                                    <option value="3">ERROR (仅错误)</option>
                                </select>
                                <button id="qr-view-logs-btn" class="menu_button" style="width: auto; align-self: stretch;">查看日志</button>
                                <button id="qr-download-logs-panel-btn" class="menu_button" style="width: auto; align-self: stretch;">下载日志</button>
                            </div>
                        </div>
                        ` : ''}
                ...
                <div id="${Constants.ID_PAGE_WHITELIST}" class="qr-settings-page">
                    ...
                    <div id="${Constants.ID_WHITELIST_HELP_BOX}" class="qr-whitelist-help-box">
                        <p><strong>直接点击上方列表中的选项即可将对应选项按钮移动到另一侧列表中。</strong></p>
                        <p><strong>在白名单列表中的选项的按钮将保留在聊天输入框上方，不会显示在QR助手的菜单面板中，而在收纳列表中的选项按钮则会出现菜单面板中，不出现在聊天输入框上方。</strong></p>
                    ...
                    <div class="qr-icon-settings-layout">
                        ...
                        <div class="qr-icon-top-bar">
                            <select id="${Constants.ID_ICON_SOURCE_SELECT}" class="text_pole" style="flex:1;">
                                ...
                                <option value="character">当前角色卡</option>
                        ...
                        <div class="qr-icon-content">
                            ...
                            <div id="${Constants.ID_PANEL_BUILTIN}" class="qr-icon-panel active">
                                ...
                                <div style="display:flex; flex-direction:column; gap:5px;">
                                    <div style="display:flex; flex-direction:row; gap:5px; align-items:center;">
                                        <label style="font-weight:bold; min-width: 70px;">图标大小:</label>
                                        <select id="${Constants.ID_BUILTIN_SIZE_SELECT}" class="text_pole" style="flex:1;">
                                            <option value="match">与发送按钮保持一致大小</option>
                                            <option value="custom">自定义内置图标大小</option>
                                        </select>
                                    </div>
                                    <div id="${Constants.ID_BUILTIN_SIZE_INPUT}-wrapper" style="display:none; flex-direction:row; gap:5px; align-items:center;">
                                        <label style="font-weight:bold; min-width: 70px;">大小 (em):</label>
                            ...
                            <div id="${Constants.ID_PANEL_CHARACTER}" class="qr-icon-panel">
                                <p style="text-align:center; opacity:0.6; font-size:13px; margin:0;">显示当前聊天角色的头像作为按钮图标，切换聊天时自动更新</p>
        ...
        savedIcons.forEach((icon, index) => {
            ...
            <div class="qr-manage-item" data-index="${index}">
                ...
                <div class="qr-manage-actions">
                    <!-- 改为调用 editIcon -->
                    <button class="qr-icon-btn edit" title="编辑" onclick="window.quickReplyMenu.editIcon(${index})">
...
function createAddModalHtml() {
    ...
    <div class="qr-modal-box" id="${Constants.ID_MODAL_ADD}">
        ...
        <div class="qr-input-group">
            <label>图标大小 (默认值1.2表示与发送按钮保持一致大小。因此非必要请不要修改该值，除非您确实想要强制调大或调小该图标):</label>
...
function createEditModalHtml(icon) {
    ...
    return `
    <div class="qr-modal-box" id="${Constants.ID_MODAL_ADD}">
        <div class="qr-modal-header">编辑图标</div>
        ...
        <div class="qr-modal-footer">
            <button id="qr-btn-save-edit" class="menu_button primary">保存修改</button>
            <button id="qr-btn-cancel-edit" class="menu_button">返回</button>
        </div>
    </div>`;
}
...
export function updateBuiltinPreview(type) {
    ...
    el.innerHTML = `<i class="fa-solid ${iconClass}" style="${sizeStyle}"></i><span class="qr-icon-preview-label">预览</span>`;
}
...
export function updateCharacterPreview() {
    ...
    const avatarImg = document.querySelector("#avatar_load_preview");
    if (avatarImg && avatarImg.src) {
        el.innerHTML = `<img src="${avatarImg.src}" class="qr-character-preview-avatar"><span class="qr-icon-preview-label">预览</span>`;
    } else {
        el.innerHTML = `<span>无角色头像</span><span class="qr-icon-preview-label">预览</span>`;
    }
}
    ...
    <div class="qr-modal-box qr-help-mode" id="${Constants.ID_MODAL_HELP}">
        ...
        <div class="qr-help-header">
            <div class="qr-help-header-left">
                <button class="qr-help-sidebar-toggle" id="${Constants.ID_BTN_TOGGLE_SIDEBAR}" title="目录">
        ...
        <div class="qr-help-split-layout">
            ...
            <div class="qr-help-sidebar">
                <div class="qr-toc-item active" data-target="help-video">视频示例</div>
                <div class="qr-toc-item" data-target="help-core">核心功能</div>
...
function createChangelogModalHtml(changelogText) {
    ...
    <div class="qr-modal-box" id="${Constants.ID_MODAL_UPDATE}">
        ...
        <div class="qr-update-tip">如果更新失败，可以在酒馆扩展页面的【管理扩展程序】列表中手动更新QR助手。如果依然更新失败，则可以尝试删除重装来使用最新版本插件。</div>
        <div class="qr-modal-footer">
...
function createLogModalHtml() {
    ...
    <div class="qr-modal-box" id="${Constants.ID_MODAL_LOG}" style="width: 800px; height: 80vh; max-height: 90vh;">
        ...
        <div class="qr-modal-header" style="display: flex; justify-content: space-between; align-items: center;">
            <span style="font-size: 16px; font-weight: bold;">运行日志</span>

            <!-- 右侧按钮组容器 -->
            <div style="display: flex; gap: 8px; align-items: center;">
                <button id="${Constants.ID_BTN_COPY_LOG}" class="menu_button qr-log-header-btn">复制</button>
                <button id="${Constants.ID_BTN_DOWNLOAD_LOG}" class="menu_button qr-log-header-btn">下载</button>
                <button id="${Constants.ID_BTN_CLEAR_LOG}" class="menu_button qr-log-header-btn" style="color: #d32f2f; border-color: rgba(211, 47, 47, 0.3);">清空</button>

                <!-- 竖线分隔符 -->
                <div style="width: 1px; height: 18px; background: #ccc; margin: 0 6px;"></div>

                <!-- 关闭按钮 -->
                <button class="qr-help-close-btn" onclick="document.getElementById('${Constants.ID_MODAL_OVERLAY}').classList.remove('visible')" title="关闭" style="height: 24px; width: 24px; display: flex; align-items: center; justify-content: center;">
                ...
                try {
                    ...
                    if (log.data.stack) {
                        ...
                        <div style="margin-top: 4px; font-size: 0.85em; opacity: 0.8;">
                            <details>
                                <summary style="cursor: pointer; text-decoration: underline;">查看错误堆栈 (Stack Trace)</summary>
